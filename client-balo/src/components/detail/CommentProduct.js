...
function CommentProduct(props) {
  ...
  const Comment = () => {
    ...
    else alert('Bạn ơi! Bạn đăng nhập rồi hãy nhận xét nha bạn!!!')
  }
  useEffect(() => {
      ...
      <div className='comment'>
        <Col span={18} align='start' style={{ alignItems:'center'}} xs={24} sm={24} md={18}>
          <div className="comment-area" style={{display: 'flex', alignItems:'center'}}>
            <textarea placeholder='Xin mời để lại câu hỏi, Balo Xinh sẽ trả lời trong 1h từ 7h - 22h mỗi ngày.' rows={10} cols={3} value={value} onChange={(e) => setValue(e.target.value)}></textarea>
          </div>
